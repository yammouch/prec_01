...
void dump_1d(long x, long len) {
  long i;
  for (i = len-1; 0 <= i; i--) printf("%d", x & (1 << i));
  printf("\n");
  return;
}
...
long measure_field_3_height() {
  long i = 0, j;
  while (field_3 == 0 && i++ < 15);
  j = i;
  while (field_3 != 0 && j++ < 15);
  return j - i;
}
    ...
    for (y = 0; y < 12; y++) {
      for (x = 0; x < 12; x++) {
        if (add_part_C(orien, y, x)) {
          area = measure_field_3_height() * measure_field_3_width();
